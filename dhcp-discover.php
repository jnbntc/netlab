...
<div class="card shadow">
  <div class="card-header bg-dark text-white">Descubridor de Servidores DHCP por VLAN</div>
  <div class="card-body">
    <p class="card-text">Verifica qué servidor DHCP está respondiendo en una interfaz de VLAN específica.</p>
    <?php
...
if (!empty($_POST["interface"])) {
    ...
    // Limpiar el nombre de la interfaz para que nmap lo entienda (ej: de 'eth0.10@eth0' a 'eth0.10')
    $parts = explode('@', $_POST['interface']);
    $interface_for_command = escapeshellarg($parts[0]);
    // --- FIN DE LA CORRECCIÓN ---

    $command = "sudo /usr/bin/nmap --script broadcast-dhcp-discover -e $interface_for_command";
  ...
  $temp_file = '/tmp/netlab_result_' . session_id() . '.txt';
    ...
    if ($return_code === 0) {
        ...
        echo '<div class="card mt-3"><div class="card-body"><h5 class="card-title">Exportar Resultado</h5><a href="export.php?format=txt&tool=dhcp-discover" class="btn btn-secondary">Exportar a TXT</a><a href="export.php?format=csv&tool=dhcp-discover" class="btn btn-success">Exportar a CSV</a><a href="export.php?format=pdf&tool=dhcp-discover" class="btn btn-danger">Exportar a PDF</a></div></div>';
    } else { 
